...
local ims = torch.FloatTensor(opt.batchSize, 3, opt.side, opt.side)
...
function train(datasetExtractor, model, logsoft, loss, dropout, top5, epoch)
   ...
   for batch = 1, nb_batches do
      ...
      local eval_E = function (att)
         ...
         local outputModel = model:forward(ims)
         local preds = logsoft:forward(outputModel)
         ...
         local gradLogSoft = logsoft:backward(outputModelCPU, dE_dy)
