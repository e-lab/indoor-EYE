...
local datasetExtractor = dmanager.TrainingExtractor(opt.dataset, trainOpt)
...
if opt.network == 'N/A' then
   model, logsoft, loss, dropout = get_model1(#datasetExtractor:getClasses(), statFile, false) --(classifier.lua)
else
   if (verbose) then
      print('Loading network from file: ' .. opt.network)
   end
   model, logsoft, loss, dropout = get_model2(opt.network, false)
end
